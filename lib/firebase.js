...
export const testRTDBConnection = async () => {
  ...
  if (!instance || !instance.rtdb) {
    return { success: false, error: "Firebase RTDB belum terinisialisasi." };
  }
  const config = getStoredFirebaseConfig();
  try {
    const pingRef = ref(instance.rtdb, "ping_test");
    await set(pingRef, {
      ping: "ok",
    ...
    return {
      success: true,
      message:
        "Sukses! Data berhasil ditulis langsung ke Firebase Realtime Database.",
