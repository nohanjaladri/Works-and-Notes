...
export const parseRosterText = (text, fallbackYearMonth) => {
  ...
  for (let rawLine of lines) {
    ...
    line = line
      ...
      .replace(/\|/g, " ")
      .trim();
    // Pattern 1: Full Date format YYYY-MM-DD (e.g. 2026-09-05: OFF or 2026-09-05, PAGI)
    let fullMatch = line.match(/(\d{4}-\d{2}-\d{2})\s*[:=,\-\t\s|]+\s*([a-zA-Z]+)/i);
    
    // Pattern 2: Short day format (e.g. 05: OFF or 5: PAGI or 1. 05: PAGI)
    let shortMatch = !fullMatch ? line.match(/(?:^|[^0-9])(\d{1,2})\s*[:=,\-\t|]+\s*([a-zA-Z]+)/i) : null;
    ...
    if (match) {
      ...
      const shiftCodeRaw = match[2].toUpperCase().trim();
      ...
      } else if (shiftCodeRaw.startsWith("O") || shiftCodeRaw.startsWith("L") || shiftCodeRaw === "OFF") {
        shift = "OFF";
      }
